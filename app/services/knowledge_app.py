...
def get_default_kb() -> FinancialKnowledgeBase:
    ...
    - FINREGQA_EMBEDDING_MODEL / FINREGQA_MILVUS_URI / FINREGQA_MILVUS_TOKEN / FINREGQA_MILVUS_COLLECTION / FINREGQA_MAX_CONNECTIONS / FINREGQA_EMBEDDING_DIM
    """
    ...
    _DEFAULT_KB = FinancialKnowledgeBase(
        ...
        db_password=os.getenv("FINREGQA_DB_PASSWORD", "password"),
        embedding_model=os.getenv("FINREGQA_EMBEDDING_MODEL", "BAAI/bge-small-zh-v1.5"),
        milvus_uri=os.getenv("FINREGQA_MILVUS_URI", "http://127.0.0.1:19530"),
        milvus_token=os.getenv("FINREGQA_MILVUS_TOKEN", ""),
        milvus_collection_name=os.getenv("FINREGQA_MILVUS_COLLECTION", "financial_knowledge"),
...
def example_basic_usage():
    ...
    kb = FinancialKnowledgeBase(
        ...
        milvus_collection_name="financial_knowledge",
        max_connections=10,
        embedding_dim=768
    )
...
if __name__ == "__main__":
    ...
    print("金融知识库系统 - 使用示例")
    print("=" * 80)
    print("\n注意: 请确保MySQL与Milvus已启动并配置正确的连接参数")
    print("=" * 80)
    ...
    try:
        ...
        print(f"\n✗ 执行出错: {e}")
        print("\n请检查:")
        print("1. MySQL服务是否正在运行，Milvus是否可连接")
        print("2. 数据库连接参数是否正确")
